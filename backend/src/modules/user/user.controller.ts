...
import { updateProfileSchema, avatarSchema } from "./user.schema.js";
...
export const userController = {
  ...
  uploadAvatar: async (req: AuthRequest, res: Response) => {
    ...
    // Valida o arquivo com o zod (tamanho, formato e existência)
    avatarSchema.parse({ file: req.file });
    const file = req.file!;

    try {
      const updatedUser = await userService.uploadAvatar(userId, file);
      return res.status(200).json(new ApiResponse({ user: updatedUser }, "Avatar atualizado com sucesso"));
    } catch (error: any) {
